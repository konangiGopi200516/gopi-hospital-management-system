...
const services = [
  { title: 'Retina & Vitreous', icon: Activity, total: 7, avail: 7 },
  { title: 'Cataract Services', icon: Droplet, total: 9, avail: 9 },
  { title: 'LASIK & Refractive', icon: Scan, total: 7, avail: 7 },
  { title: 'Glaucoma Clinic', icon: Eye, total: 7, avail: 7 },
  { title: 'Cornea Services', icon: Eye, total: 7, avail: 7 },
  { title: 'Pediatric Ophthalmology', icon: Users, total: 6, avail: 6 },
  { title: 'Neuro Ophthalmology', icon: Activity, total: 6, avail: 6 },
  { title: 'Oculoplasty', icon: Eye, total: 6, avail: 6 },
  { title: 'Eye Emergency', icon: ShieldCheck, total: 6, avail: 6 },
];

const uniqueNames = [
  'Sarah Jenkins', 'Michael Chen', 'Emily Carter', 'James Wilson', 'David Miller', 
...
const allDoctors = services.flatMap(dept => {
  return Array.from({ length: dept.total }).map((_, i) => {
    ...
    return {
      ...
      name: `Dr. ${name}`,
      email: `dr.${emailName}@visioncare.com`,
      mobile: `+91 9${Math.floor(8000 + ((globalIndex * 13) % 1999))} ${Math.floor(10000 + ((globalIndex * 31) % 89999))}`,
...
const DOCTOR_PAYROLL = allDoctors.map((doc, idx) => {
  ...
  return {
    ...
    photo: `https://randomuser.me/api/portraits/${gender}/${picId}.jpg`
  };
});
...
const PAYROLL_DATA = [...DOCTOR_PAYROLL, ...OTHER_PAYROLL];

module.exports = { DOCTOR_PAYROLL, OTHER_PAYROLL };